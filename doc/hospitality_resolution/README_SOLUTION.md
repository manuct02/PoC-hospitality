...
El **agente orquestador** es el "cerebro" del sistema que decide automáticamente cuándo usar:
- **Tools** → Para consultas estructuradas (conteos, filtros, listas exactas)
- **RAG** → Para búsquedas semánticas (descripciones, contexto cualitativo)
...
def create_hotel_agent():
    ...
    agent_config = get_agent_config()
    llm = ChatGoogleGenerativeAI(
        model=agent_config.model,          # gemini-2.5-flash-lite
        temperature=0,                     # Determinístico (sin creatividad)
        google_api_key=agent_config.api_key
    )

    # 2. Definir las tools disponibles
    tools = [
        search_hotels_by_city,  # Tool 1: Buscar por ciudad
        list_all_hotels,        # Tool 2: Listar todos
        count_rooms,            # Tool 3: Contar habitaciones
        get_room_prices         # Tool 4: Obtener precios
    ]

    # 3. Bind tools al LLM (forma moderna de LangChain 0.2+)
...
- **Paso 1: Configurar el LLM**
  - Modelo:  Gemini 2.5 Flash Lite (rápido y eficiente).
  - Temperature= 0 $\longrightarrow$ Sin creatividad, sólo precisión.
  - Gemini API Key

- **Paso 2: Definir las Tools**

Cada función con `@tool` se convierte en un objeto `StructuredTool` que el LLM puede descubrir y usar.

- **Paso 3: Bind Tools**

  - Toma el LLM base
  - Le "adjunta" las 4 herramientas
  - Ahora el LLm puede:
    - Ver las funciones (`@tools`) disponibles
    - Leer las descripciones (docstrings)
    - Decidir cuándo llamarlas
    - Generar `tool_calls`en su respuesta

De esta forma tenemos un LLM "potenciado" que sabe que tiene tools disponibles a su uso.

### Orquestrador principal:

Esta función es el **corazón del sistema**. Decide:
...
El **agente orquestrador** en `hotel_rag_agent.py` es:

- 1 -  `create_hotel_agent()`:
  - Crea LLM + Tools
  - Devuelve `llm_with_tools`

- 2 - `invoke_agent_with_tools(query)`:
  - Punto de entrada principal
  - Decide: Tool vs RAG
  - Ejecuta la acción correspondiente
  - Reformatea la respuesta
- 3 - **System Prompt**:
  - Las "reglas" del agente
  - Define cuándo usar cada tool
  - Crítico para decisiones correctas

- 4 - **Flujo de decisión**:
  - `response.tool_calls`existe $\longrightarrow$ Ejecutar tool
  - `response.tool_calls` vacío $\longrightarrow$ Usar RAG

- 5 - **Reformateo**:
  - Tools devuelven datos crudos
  - LLM los convierte en lenguaje natural
  - Usuario ve respuestas conversacionales

**Es el cerebro que coordina todo el sistema híbrido**.
